    ...
    execute: function () {
      ...
      _export("GameArea", GameArea = (_dec = ccclass('GameArea'), _dec2 = property({
        ...
        testBlock(pos, block) {
          ...
          this.testBlockIndex(index, block);
        }

        testBlockIndex(index, block) {
          ...
          console.log(list);
          var indexRow = Math.floor(index / 8);
          var canAdd = true;
          ...
          for (var i = 0; i < list.length; i++) {
            ...
            console.log(this.grids[index + list[i]].getComponent(_crd && Grid === void 0 ? (_reportPossibleCrUseOfGrid({
              error: Error()
            }), Grid) : Grid).Filled);
        ...
        checkGameOver() {
          ...
          for (var j = 0; j < (_crd && GameController === void 0 ? (_reportPossibleCrUseOfGameController({
            error: Error()
          }), GameController) : GameController).Instance.blocks.length; j++) {
            if ((_crd && GameController === void 0 ? (_reportPossibleCrUseOfGameController({
              error: Error()
            }), GameController) : GameController).Instance.blockUsing[j] == 1) {
              continue;
            }

            for (var i = 0; i < this.grids.length; i++) {
              var result = this.testBlockIndex(i, (_crd && GameController === void 0 ? (_reportPossibleCrUseOfGameController({
                error: Error()
              }), GameController) : GameController).Instance.blocks[j].getBlockData());
              times++;
...
//# sourceMappingURL=9aa691f68b300c621deffa682c0d2ea9087dde42.js.map